...
def recommend_agents(profile: ProjectProfile) -> list[dict]:
    ...
    # Backend Dev
    framework = next((f for f in profile.frameworks if f not in [
    ...
    if has_frontend:
        fe_framework = next(
            (f for f in profile.frameworks if f in [
                "react", "vue", "nextjs", "nuxt", "angular", "svelte",
            ]),
            "frontend",
        )
    ...
    if has_frontend:
        ...
        stages = [
            {"name": "plan", "agent": "architect",
             "gate": {"type": "approval"}, "max_iterations": 10},
            {"name": "backend", "agent": "backend-dev", "depends_on": "plan",
             "gate": {"type": "automated", "checks": ["tests_pass"]}, "max_iterations": 8},
            {"name": "frontend", "agent": "frontend-dev", "depends_on": "plan",
             "gate": {"type": "automated", "checks": ["tests_pass"]}, "max_iterations": 8},
            {"name": "review", "agent": "reviewer", "depends_on": ["backend", "frontend"],
             "gate": {"type": "score", "min_score": 80, "on_fail": "route_to", "max_retries": 2},
             "max_iterations": 3},
            {"name": "e2e", "agent": "tester", "depends_on": "review",
             "gate": {"type": "automated", "checks": ["tests_pass"]}, "max_iterations": 5},
            {"name": "deliver", "agent": "integrator", "depends_on": "e2e",
             "gate": {"type": "approval"}},
        ]
        ...
        stages = [
            ...
            {"name": "backend", "agent": "backend-dev", "depends_on": "plan",
             "gate": {"type": "automated", "checks": ["tests_pass"]}, "max_iterations": 8},
            {"name": "review", "agent": "reviewer", "depends_on": "backend",
             "gate": {"type": "score", "min_score": 80, "on_fail": "route_to", "max_retries": 2},
             "max_iterations": 3},
            {"name": "e2e", "agent": "tester", "depends_on": "review",
             "gate": {"type": "automated", "checks": ["tests_pass"]}, "max_iterations": 5},
        ...
        stages = [
            {"name": "plan", "agent": "architect",
             "gate": {"type": "approval"}, "max_iterations": 10},
            {"name": "backend", "agent": "backend-dev", "depends_on": "plan",
             "gate": {"type": "approval"}, "max_iterations": 8},
            {"name": "review", "agent": "reviewer", "depends_on": "backend",
             "gate": {"type": "score", "min_score": 80, "max_retries": 1},
             "max_iterations": 3},
            {"name": "deliver", "agent": "integrator", "depends_on": "review",
             "gate": {"type": "approval"}},
        ]
